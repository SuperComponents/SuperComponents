...
export class TailwindConfigGenerator {
  ...
  private extractColorValues(tokens: W3CDesignTokens, target: { [key: string]: string }): void {
  ...
  private mapTypographyTokens(typographyTokens: W3CDesignTokens, theme: any): void {
  ...
  private mapBorderRadiusTokens(borderRadiusTokens: W3CDesignTokens, theme: any): void {
    theme.borderRadius = {};
  ...
  private mapTransitionTokens(transitionTokens: W3CDesignTokens, theme: any): void {
    Object.entries(transitionTokens).forEach(([key, value]) => {
      switch (key) {
        case 'duration':
          theme.transitionDuration = {};
          this.extractTokenValues(value as W3CDesignTokens, theme.transitionDuration);
          break;
        case 'timingFunction':
          theme.transitionTimingFunction = {};
          this.extractTokenValues(value as W3CDesignTokens, theme.transitionTimingFunction);
          break;
      }
    });
  }

  private extractTokenValues(tokens: W3CDesignTokens, target: { [key: string]: any }): void {
    Object.entries(tokens).forEach(([key, value]) => {
      if (typeof value === 'object' && '$value' in value) {
        target[key] = (value as W3CDesignToken).$value;
      }
...
`;
  ...
  private generateTokenUtilities(tokens: W3CDesignTokens, utilities: { [key: string]: { [key: string]: string } }, prefix = ''): void {
    Object.entries(tokens).forEach(([key, value]) => {
      ...
      if (typeof value === 'object' && '$value' in value) {
        ...
        this.generateTokenUtilities(value as W3CDesignTokens, utilities, utilityName);
      }
    });
  }
  ...
  private extractCSSVariables(tokens: W3CDesignTokens, cssVars: string[], prefix = ''): void {
    Object.entries(tokens).forEach(([key, value]) => {
