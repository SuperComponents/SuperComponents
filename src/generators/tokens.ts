...
export class TokenGenerator {
  ...
  private generateSpacingTokens(insight: DesignInsight): W3CDesignTokens {
    ...
    // Base spacing scale
    const baseScale =
      insight.spacingScale.length > 0
        ? insight.spacingScale
    ...
    spacing.xs = {
      $type: 'spacing',
      $value: `${Math.round(4 * densityMultiplier)}px`,
    }
    spacing.sm = {
      $type: 'spacing',
      $value: `${Math.round(8 * densityMultiplier)}px`,
    }
    spacing.md = {
      $type: 'spacing',
      $value: `${Math.round(16 * densityMultiplier)}px`,
    }
    spacing.lg = {
      $type: 'spacing',
      $value: `${Math.round(24 * densityMultiplier)}px`,
    }
    spacing.xl = {
      $type: 'spacing',
      $value: `${Math.round(32 * densityMultiplier)}px`,
    }
    spacing['2xl'] = {
      $type: 'spacing',
      $value: `${Math.round(40 * densityMultiplier)}px`,
    }
    spacing['3xl'] = {
      $type: 'spacing',
      $value: `${Math.round(48 * densityMultiplier)}px`,
    }
  ...
  private generateSizingTokens(insight: DesignInsight): W3CDesignTokens {
    ...
    // Base sizes
    const baseSizes = [
      16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 64, 72, 80, 96,
    ]
    ...
    sizing.xs = {
      $type: 'sizing',
      $value: `${Math.round(20 * densityMultiplier)}px`,
    }
    sizing.sm = {
      $type: 'sizing',
      $value: `${Math.round(24 * densityMultiplier)}px`,
    ...
    sizing.md = {
      $type: 'sizing',
      $value: `${Math.round(28 * densityMultiplier)}px`,
    }
    sizing.lg = {
      $type: 'sizing',
      $value: `${Math.round(32 * densityMultiplier)}px`,
    ...
    sizing.xl = {
      $type: 'sizing',
      $value: `${Math.round(36 * densityMultiplier)}px`,
    }
  ...
  private generateBorderRadiusTokens(insight: DesignInsight): W3CDesignTokens {
    ...
    const baseRadius =
      insight.uiDensity === 'compact'
        ? 2
        : insight.uiDensity === 'spacious'
          ? 8
          : 4
    ...
    borderRadius.sm = {
      $type: 'borderRadius',
      $value: `${Math.round(baseRadius * 0.5)}px`,
    }
    borderRadius.md = { $type: 'borderRadius', $value: `${baseRadius}px` }
    borderRadius.lg = {
      $type: 'borderRadius',
      $value: `${Math.round(baseRadius * 2)}px`,
    }
    borderRadius.xl = {
      $type: 'borderRadius',
      $value: `${Math.round(baseRadius * 3)}px`,
    }
    borderRadius['2xl'] = {
      $type: 'borderRadius',
      $value: `${Math.round(baseRadius * 4)}px`,
    }
  ...
  convertToLegacyFormat(w3cTokens: W3CDesignTokens): DesignTokens {
    ...
    // Extract colors
    if (w3cTokens.color) {
      this.extractColorsFromW3C(
        w3cTokens.color as W3CDesignTokens,
        legacy.colors
      )
    }
  ...
  private extractColorsFromW3C(
    colorTokens: W3CDesignTokens,
      ...
      Object.entries(semanticColors).forEach(([key, value]) => {
        if (typeof value === 'object' && '$value' in value) {
          ...
          if (contrast < minRatio) {
            ...
            const adjustedColor = this.adjustColorForContrast(
              colorValue,
              whiteBackground,
              minRatio
            )
  ...
  validateContrast(color1: string, color2: string): ContrastResult {
    ...
    if (!passes) {
      adjustedColor = this.adjustColorForContrast(
        color1,
        color2,
        this.options.minContrastRatio || 4.5
      )
    }
