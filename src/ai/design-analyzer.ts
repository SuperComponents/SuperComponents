...
export class AIDesignAnalyzer {
  ...
  async analyzeInspiration(input: InspirationInput): Promise<AIAnalysisResult> {
    ...
    const analysisResult = await this.generateDesignSystemFromInsights(
      insights,
      input
    )
  ...
  private async extractDesignInsights(
    ...
    try {
      ...
      throw new Error(
        `Failed to extract design insights: ${error instanceof Error ? error.message : String(error)}`
      )
    }
  }
...
Return only the JSON object, no additional text.`
  ...
  private async callVisionAPI(
    ...
    if (!this.openai) {
      // Return a default response when OpenAI is not available
      return JSON.stringify({
        imageryPalette: ['#3b82f6', '#10b981', '#f59e0b', '#ef4444'],
        typographyFamilies: ['Inter', 'system-ui'],
  ...
  private parseInsightsResponse(response: string): DesignInsight {
    try {
      ...
      throw new Error(
        `Failed to parse insights response: ${error instanceof Error ? error.message : String(error)}`
      )
    }
  }
  ...
  private convertTypographyToTokens(
    families: string[]
  ): DesignTokens['typography'] {
    return {
      fonts:
        families.length > 0 ? families : ['Inter', 'system-ui', 'sans-serif'],
      sizes: {
        xs: '0.75rem',
        sm: '0.875rem',
        base: '1rem',
  ...
  private generateBorderRadiusTokens(
    density: DesignInsight['uiDensity']
  ...
  private generateShadowTokens(
    ...
    const shadowIntensity =
      density === 'compact' ? 0.5 : density === 'regular' ? 1 : 1.5
  ...
  private inferTargetAudience(keywords: string[]): string {
    ...
    if (keywords.some((k) => professionalKeywords.includes(k.toLowerCase()))) {
      ...
      keywords.some((k) => creativeKeywords.includes(k.toLowerCase()))
    ) {
  ...
  private generateRecommendations(insights: DesignInsight): string[] {
    ...
    if (insights.typographyFamilies.length > 1) {
      recommendations.push(
        'Consider using primary font for headings and secondary for body text'
      )
    }

    if (
      insights.brandKeywords.includes('modern') ||
      insights.brandKeywords.includes('minimal')
    ) {
      recommendations.push(
        'Embrace white space and clean lines in component design'
      )
    }
  ...
  private async persistInsights(
    ...
    try {
      ...
      console.warn(
        `Failed to persist insights: ${error instanceof Error ? error.message : String(error)}`
      )
    }
  }
