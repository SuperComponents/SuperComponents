...
describe('AIDesignAnalyzer', () => {
  ...
  describe('analyzeInspiration', () => {
    it('should analyze modern dashboard design', async () => {
      ...
      const result = await analyzer.analyzeInspiration({
        ...
        source: path.join(
          process.cwd(),
          'fixtures/inspiration/modern-dashboard.png'
        ),
        context: 'Analytics dashboard for business users',
      })

      expect(result.insights.imageryPalette).toEqual([
        '#3B82F6',
        '#10B981',
        '#F59E0B',
        '#EF4444',
      ...
      expect(result.insights.spacingScale).toEqual([
      ...
      expect(result.extractedTokens.typography.fonts).toEqual([
        'Inter',
        'Roboto',
      ])
      ...
      expect(result.inferredPrinciples.targetAudience).toContain(
    ...
    it('should analyze nature blog design', async () => {
      ...
      const result = await analyzer.analyzeInspiration({
        ...
        source: path.join(
          process.cwd(),
          'fixtures/inspiration/nature-blog.png'
        ),
      ...
      expect(result.insights.typographyFamilies).toEqual([
        'Merriweather',
        'Open Sans',
      ])
      expect(result.insights.spacingScale).toEqual([
      ...
      expect(result.inferredPrinciples.coreValues).toEqual([
        'natural',
        'organic',
        'peaceful',
      ])
    ...
    it('should analyze luxury ecommerce design', async () => {
      ...
      const result = await analyzer.analyzeInspiration({
        ...
        source: path.join(
          process.cwd(),
          'fixtures/inspiration/luxury-ecommerce.png'
        ),
    ...
    it('should handle text-based inspiration', async () => {
      ...
      const result = await analyzer.analyzeInspiration({
        type: 'text',
        source:
          'A developer-focused application with technical documentation and code examples',
  ...
  describe('Design Token Conversion', () => {
    ...
    it('should generate density-appropriate border radius', () => {
      const compactTokens = (analyzer as any).generateBorderRadiusTokens(
        'compact'
      )
      const spaciousTokens = (analyzer as any).generateBorderRadiusTokens(
        'spacious'
      )
